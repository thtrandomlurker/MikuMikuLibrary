...
public class ObjectSet : BinaryFile
{
    ...
    public override void Write(EndianBinaryWriter writer, ISection section = null)
    {
        ...
        writer.WriteOffset(16, AlignmentMode.Center, () =>
        {
            foreach (var obj in Objects)
            {
                writer.WriteOffsetIf(section == null, 4, AlignmentMode.Center, () =>
                {
        ...
        {
            foreach (var obj in Objects)
                writer.WriteOffsetIf(obj.Skin != null && section == null, 16, AlignmentMode.Left, () => { obj.Skin.Write(writer, Format); });
        });
        writer.WriteOffset(16, AlignmentMode.Center, () =>
        {
            foreach (var obj in Objects)
                writer.WriteOffset(16, AlignmentMode.Left, () => writer.Write(obj.Name, StringBinaryFormat.NullTerminated));
        });
        writer.WriteOffset(16, AlignmentMode.Center, () =>
        {
            foreach (var obj in Objects)
    ...
    public void Save(Stream destination, ObjectDatabase objectDatabase, TextureDatabase textureDatabase, BoneData boneData, bool leaveOpen = false)
    {
        foreach (var obj in Objects)
    ...
    public void Save(string filePath, ObjectDatabase objectDatabase, TextureDatabase textureDatabase, BoneData boneData)
    {
        ...
        using (var destination = File.Create(filePath))
            Save(destination, objectDatabase, textureDatabase, boneData);
    ...
    public void TryFixParentBoneInfos(BoneData boneData = null)
    {
        foreach (var obj in Objects)
            obj.Skin?.TryFixParentBoneInfos(boneData?.Skeletons.FirstOrDefault(x => obj.Name.StartsWith(x.Name, StringComparison.OrdinalIgnoreCase)));
    }
